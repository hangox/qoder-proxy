...
export type QoderExpectedModelIdentity = { key: string; displayName: string };

export function expectedModelForRoutingKey(routingKey: string): QoderExpectedModelIdentity | undefined {
  const entry = Object.values(QODER_TIER_REGISTRY).find((candidate) => candidate.routingKey === routingKey);
  return entry === undefined ? undefined : { key: entry.routingKey, displayName: entry.displayName };
}

export function hasExpectedModelIdentity(model: Pick<QoderAssistantModel, "key" | "displayName">, routingKey: string): boolean {
  const expected = expectedModelForRoutingKey(routingKey);
  return expected !== undefined && model.key === expected.key && model.displayName === expected.displayName;
}
...
export class QoderModelUnavailableError extends Error {
  ...
  readonly reason: "missing" | "identity-mismatch";
  constructor(routingKey: string, reason: "missing" | "identity-mismatch" = "missing") {
    super(`Qoder runtime model unavailable: ${routingKey}${reason === "identity-mismatch" ? " (identity mismatch)" : ""}`);
