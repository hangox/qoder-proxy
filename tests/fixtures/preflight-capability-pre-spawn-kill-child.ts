...
const [configDir, supervisorReadyPath, workerReadyPath, bodiesPath] = process.argv.slice(2);
if (!configDir || !supervisorReadyPath || !workerReadyPath || !bodiesPath) throw new Error("缺少 capability pre-spawn parent-kill 测试参数");
const env = {
...
const store = createConfigStore("machine-a", env, {
  ...
  capabilityProbeReadyPath: workerReadyPath,
  capabilitySupervisorReadyPath: supervisorReadyPath,
  capabilitySupervisorStartupDelayMs: 500,
});
